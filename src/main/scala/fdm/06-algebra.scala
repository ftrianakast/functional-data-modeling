...
object algebra {
  ...
  def toAB[A, B](ba: Either[B, A]): Either[A, B] = toBA(ba)
   ...
   */
  def withUnit[A](v: A): (A, Unit)    = (v, Unit)
...
object algebra_of_types {
   ...
   */
  sealed trait APlusB[+A, +B]
  object APlusB {
    final case class A[+A](a: A) extends APlusB[A, Nothing]
    final case class B[+B](b: B) extends APlusB[Nothing, B]
  }
